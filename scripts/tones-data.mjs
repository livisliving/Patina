/**
 * The measured Aqua materials and the five tones — the data behind
 * scripts/tones.mjs (which writes them into y2k.css) and scripts/figma/
 * (which writes the same rows into the Figma library). One source; every
...
export const toHex = (c) => "#" + c.map((v) => Math.round(Math.min(255, Math.max(0, v))).toString(16).padStart(2, "0")).join("")
export const lin = (v) => ((v /= 255) <= 0.04045 ? v / 12.92 : ((v + 0.055) / 1.055) ** 2.4)
export const delin = (v) => 255 * (v <= 0.0031308 ? 12.92 * v : 1.055 * Math.max(0, v) ** (1 / 2.4) - 0.055)
...
export const mix = (c, t, to = [255, 255, 255]) => c.map((v, i) => v + (to[i] - v) * t)
...
export const ACCENT = hex("#4d83d2") // aqua-ui's accent / selection blue
export const ACCENT_LCH = oklch(ACCENT)
...
export const BARBER = [["#feffff", 0.0], ["#feffff", 0.71], ["#fdfeff", 1.41], ["#fbfdff", 2.12], ["#f5faff", 2.83], ["#ebf5ff", 3.54], ["#dcedff", 4.24], ["#c7e1ff", 4.95], ["#add4ff", 5.66], ["#8fc4fe", 6.36], ["#75b7fe", 7.07], ["#60abfe", 7.78], ["#51a3fe", 8.49], ["#479efe", 9.19], ["#419bfe", 9.9], ["#3f9afe", 10.61], ["#3e99fe", 11.31], ["#3e99fe", 12.02], ["#3f9afe", 12.73], ["#419bfe", 13.44], ["#479efe", 14.14], ["#51a3fe", 14.85], ["#60abfe", 15.56], ["#75b7fe", 16.26], ["#8fc4fe", 16.97], ["#add4ff", 17.68], ["#c7e1ff", 18.38], ["#dcedff", 19.09], ["#ebf5ff", 19.8], ["#f5faff", 20.51], ["#fbfdff", 21.21], ["#fdfeff", 21.92], ["#feffff", 22.63]]

// Progress ribs, one 16px period (white above the static fill, black below).
export const RIBS = [["rgba(255,255,255,0.095)", 0], ["rgba(255,255,255,0.026)", 3], ["rgba(0,0,0,0)", 4], ["rgba(0,0,0,0.035)", 6], ["rgba(0,0,0,0.057)", 8], ["rgba(0,0,0,0.03)", 10], ["rgba(0,0,0,0)", 12.5], ["rgba(255,255,255,0.043)", 14], ["rgba(255,255,255,0.095)", 16]]
// Barber-pole lighting per row, as black over the unshaded stripes (row 14 = 0).
export const BARBER_SHADE = [0.548, 0.297, 0.11, 0.028, 0.015, 0.046, 0.105, 0.173, 0.226, 0.234, 0.195, 0.138, 0.076, 0.029, 0, 0, 0, 0.031].map((a) => `rgba(0,0,0,${a})`)
...
export const fmt = ([c, a]) => (a >= 1 ? toHex(c) : `rgba(${c.map((v) => Math.round(Math.min(255, Math.max(0, v)))).join(", ")}, ${a})`)
...
export function retone(colour, base) {
...
export function tint(colour, base) {
  const [, Ca] = ACCENT_LCH
...
export const toned = (s, base, exact) => {
  if (exact) return s
  const [c, a] = parse(s)
  const [L, C] = oklch(c)
...
export const contrast = (a, b) => (Math.max(lum(a), lum(b)) + 0.05) / (Math.min(lum(a), lum(b)) + 0.05)
/** White or black — whichever reads on EVERY row of the highlight. */
export function inkFor(list, base, exact) {
...
export const reads = (c, alpha = 1) => contrast([255, 255, 255], c.map((v) => v * alpha + 255 * (1 - alpha))) >= 4.5
/** The colour darkened in its own hue, a step at a time, until `ok` holds. */
export function deepen(c, ok) {
  let k = 0
 ...
 *  `one` that re-expresses a measured row in it, and the AA decisions —
 *  the selection deepened until white ink reads on it, and the sidebar's
 *  gradient base and depth. White ink has to read on the selection and down
 *  the whole sidebar gradient; where the tone's own colours miss AA they are
 *  deepened in their hue just enough, and the sidebar takes the shallow
 *  gradient. */
