...
function DesktopShell({ wallpaper, IPod, className }: { wallpaper?: DesktopProps["wallpaper"]; IPod?: IPodComponent; className?: string }) {
  ...
  React.useEffect(() => {
    ...
    if (linked.current === "arrived") {
      linked.current = null
      if (frontId && !window.matchMedia("(min-width: 768px)").matches)
  ...
  return (
    <div className={cn("min-h-dvh overflow-x-hidden font-(family-name:--y2k-font-ui) text-(--y2k-ink)", className)}>
      ...
      {isDesktop && (
        <div aria-hidden className="absolute inset-0 top-(--y2k-menubar-h) z-0 hidden md:block" {...rootProps}>
          <Band rect={band} />
        </div>
      )}
      ...
      <main className="relative isolate z-[2] flex flex-col gap-5 px-3 pt-8 pb-24 md:block md:px-0 md:pt-0 md:pb-0">
        {windows.map((w) => {
          switch (w.kind) {
            case "finder":
              return (
            ...
            case "about": {
              const info = w.payload as { app: string } | undefined
              return (
                // The owner's About box is 448px — room for the panes; About <app> stays 300.
                <AboutWindow key={w.id} win={w} className={info ? undefined : "md:w-[448px]"}>
                  {info ? (
                    // About <the front app>: its icon and its name.
                    <>
            ...
            case "ipod":
              ...
              return IPod ? (
                <DesktopWindow key={w.id} win={w} material="metal" className="md:h-[400px] md:w-[600px]">
